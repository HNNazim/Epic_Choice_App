...
class EpicBestProductsAdapter : RecyclerView.Adapter<EpicBestProductsAdapter.EpicBestProductsViewHolder>() {

    inner class EpicBestProductsViewHolder(private val binding: EpicbaseRvItemBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(product: Product) {
            binding.apply {
                // Load product image
                Glide.with(itemView).load(product.images.getOrNull(0)).into(imgEpicPlacesRVItem)

                // Set product details
                tvEpicPlacesName.text = product.name
                tvEpicPlacesDetail.text = "$${product.price?.toString() ?: "N/A"}"
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): EpicBestProductsViewHolder {
        return EpicBestProductsViewHolder(
    ...
    override fun onBindViewHolder(holder: EpicBestProductsViewHolder, position: Int) {
