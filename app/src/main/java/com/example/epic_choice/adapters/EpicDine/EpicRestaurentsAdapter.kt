package com.example.epic_choice.adapters.EpicDine
...
class EpicRestaurentsAdapter: RecyclerView.Adapter<EpicRestaurentsAdapter.EpicRestaurentsViewHolder>() {

    inner class EpicRestaurentsViewHolder(private val binding: BaseepicdealsRvItemBinding): RecyclerView.ViewHolder(binding.root){
        fun bind(product: Product) {
            binding.apply {
                Glide.with(itemView).load(product.images[0]).into(imgEpicDeals)
                product.offerPercentage?.let {
                    val offPercentage = it * 100 // Assuming offerPercentage is a fractional value
                    tvNewPrice.text = String.format("%.0f%% OFF", offPercentage)
    ...
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): EpicRestaurentsViewHolder {
        return EpicRestaurentsViewHolder(
    ...
    override fun onBindViewHolder(holder: EpicRestaurentsViewHolder, position: Int) {
